//! 签名模块 - 实现星驿支付签名算法
//!
//! 本模块实现与星驿支付官方文档完全一致的签名算法
//! 签名步骤：
//! 1. 将所有参数按照ASCII码从小到大排序
//! 2. 使用URL键值对格式拼接成字符串（key1=value1&key2=value2...）
//! 3. 对拼接后的字符串进行SHA256哈希计算
//! 4. 使用RSA公钥对哈希值进行加密
//! 5. 对加密结果进行Base64编码，得到最终签名

// 引入标准库的BTreeMap，用于自动排序的键值对存储
use std::collections::BTreeMap;

// 引入标准库的result模块，用于定义Result类型
use std::result::Result;

// 引入SHA256哈希算法
use sha2::{Digest, Sha256};

// 引入RSA公钥和加密相关类型
use rsa::{Pkcs1v15Encrypt, RsaPublicKey};

// 引入RSA的DecodePublicKey trait，用于从PEM格式解析公钥
use rsa::pkcs8::DecodePublicKey;

// 引入Base64编解码
use base64::{engine::general_purpose::STANDARD, Engine};

// 引入自定义错误类型
use crate::error::SignError;

/// 签名器结构体
/// 用于生成星驿支付API请求的签名
pub struct Signer {
    /// RSA公钥
    /// 由星驿支付官方提供，用于加密签名
    public_key: RsaPublicKey,
}

/// 实现Signer结构体的方法
impl Signer {
    /// 创建新的签名器实例
    ///
    /// # 参数
    ///
    /// * `public_key_pem` - PEM格式的RSA公钥字符串
    ///
    /// # 返回值
    ///
    /// 返回Result类型：
    /// - Ok(Signer)：签名器创建成功
    /// - Err(SignError)：签名器创建失败（公钥格式错误）
    ///
    /// # 示例
    ///
    /// ```ignore
    /// let signer = Signer::new("-----BEGIN PUBLIC KEY-----...")?;
    /// ```
    pub fn new(public_key_pem: &str) -> Result<Self, SignError> {
        // 去除首尾空白字符
        let public_key_pem = public_key_pem.trim();

        // 检查是否已经是完整的PEM格式
        let pem_str = if public_key_pem.contains("-----BEGIN PUBLIC KEY-----") {
            // 已经是PEM格式，直接使用
            public_key_pem.to_string()
        } else {
            // 不是PEM格式，自动添加PEM头部和尾部
            // 将Base64内容按64字符换行，符合PEM标准格式
            let base64_with_newlines = public_key_pem
                .as_bytes()
                .chunks(64)
                .map(|chunk| std::str::from_utf8(chunk).unwrap_or(""))
                .collect::<Vec<_>>()
                .join("\n");

            format!(
                "-----BEGIN PUBLIC KEY-----\n{}\n-----END PUBLIC KEY-----",
                base64_with_newlines
            )
        };

        // 先尝试Base64解码，验证公钥内容
        // 提取PEM中的Base64内容
        let base64_content = if public_key_pem.contains("-----BEGIN PUBLIC KEY-----") {
            // 从PEM格式中提取Base64内容
            pem_str
                .lines()
                .filter(|line| !line.starts_with("-----"))
                .collect::<String>()
        } else {
            public_key_pem.to_string()
        };

        // 尝试Base64解码
        match STANDARD.decode(&base64_content) {
            Ok(decoded) => {
                // Base64解码成功，检查解码后的数据长度
                // RSA 2048位公钥的DER编码大约是294字节
                if decoded.len() < 100 {
                    return Err(SignError::RsaPublicKeyError(
                        "公钥数据太短，可能不是有效的RSA公钥".to_string()
                    ));
                }
            }
            Err(e) => {
                return Err(SignError::RsaPublicKeyError(
                    format!("Base64解码失败: {}", e)
                ));
            }
        }

        // 尝试解析PEM格式的RSA公钥
        let public_key = RsaPublicKey::from_public_key_pem(&pem_str)
            .map_err(|e| {
                // 提供更详细的错误信息
                let error_msg = format!(
                    "RSA公钥解析失败: {}\n\
                    提示：\n\
                    1. 请确保公钥是完整的Base64编码字符串\n\
                    2. 公钥应该由星驿支付官方提供\n\
                    3. 如果是PEM格式，应包含-----BEGIN PUBLIC KEY-----和-----END PUBLIC KEY-----\n\
                    4. 如果是纯Base64格式，程序会自动添加PEM标记\n\
                    当前公钥长度: {} 字符\n\
                    当前公钥前50字符: {}",
                    e,
                    public_key_pem.len(),
                    &public_key_pem.chars().take(50).collect::<String>()
                );
                SignError::RsaPublicKeyError(error_msg)
            })?;

        // 返回签名器实例
        Ok(Signer { public_key })
    }

    /// 生成签名
    ///
    /// 该方法按照星驿支付官方文档的签名算法生成签名
    ///
    /// # 签名步骤（与官方文档完全一致）
    ///
    /// 1. 将所有参数按照ASCII码从小到大排序
    /// 2. 使用URL键值对格式拼接成字符串（key1=value1&key2=value2...）
    /// 3. 对拼接后的字符串进行SHA256哈希计算
    /// 4. 使用RSA公钥对哈希值进行加密
    /// 5. 对加密结果进行Base64编码，得到最终签名
    ///
    /// # 参数
    ///
    /// * `params` - 需要签名的参数键值对
    ///
    /// # 返回值
    ///
    /// 返回Result类型：
    /// - Ok(String)：签名成功，返回签名字符串
    /// - Err(SignError)：签名失败
    pub fn sign(&self, params: &BTreeMap<String, String>) -> Result<String, SignError> {
        // ==================== 步骤1：参数排序 ====================
        // BTreeMap会自动按照键的ASCII码顺序排序
        // 因此不需要额外排序操作
        // 将参数转换为引用，避免所有权转移
        let sorted_params = params;

        // ==================== 步骤2：参数拼接 ====================
        // 使用URL键值对格式拼接成字符串
        // 格式：key1=value1&key2=value2&key3=value3...
        let concatenated = self::concat_params(sorted_params)?;

        // ==================== 步骤3：SHA256哈希计算 ====================
        // 对拼接后的字符串进行SHA256哈希计算
        // 返回小写十六进制字符串，与官方文档一致
        let hash_hex = self::calculate_sha256_hex(&concatenated)?;

        // ==================== 步骤4：RSA公钥加密 ====================
        // 使用RSA公钥对哈希字符串进行加密
        // 注意：加密的是哈希的十六进制字符串，不是字节数组
        let encrypted = self::rsa_encrypt_string(&self.public_key, &hash_hex)?;

        // ==================== 步骤5：Base64编码 ====================
        // 对加密结果进行Base64编码，得到最终签名
        let signature = self::base64_encode(&encrypted)?;

        // 返回签名结果
        Ok(signature)
    }

    /// 获取待签名的原始字符串
    ///
    /// 该方法返回步骤1和步骤2的结果，即排序后拼接的字符串
    /// 用于调试和验证签名过程
    ///
    /// # 参数
    ///
    /// * `params` - 参数键值对
    ///
    /// # 返回值
    ///
    /// 返回排序后拼接的字符串
    pub fn get_sign_string(params: &BTreeMap<String, String>) -> Result<String, SignError> {
        // 调用参数拼接方法
        concat_params(params)
    }

    /// 获取SHA256哈希值（十六进制格式）
    ///
    /// 该方法返回步骤3的结果，即SHA256哈希值
    /// 用于调试和验证签名过程
    ///
    /// # 参数
    ///
    /// * `params` - 参数键值对
    ///
    /// # 返回值
    ///
    /// 返回SHA256哈希值的十六进制字符串（小写）
    pub fn get_hash_hex(params: &BTreeMap<String, String>) -> Result<String, SignError> {
        // 先拼接参数
        let concatenated = concat_params(params)?;
        // 计算SHA256哈希，返回小写十六进制字符串
        calculate_sha256_hex(&concatenated)
    }
}

/// 参数拼接函数（内部函数）
///
/// 将参数按照URL键值对格式拼接成字符串
/// 格式：key1=value1&key2=value2&key3=value3...
///
/// # 参数
///
/// * `params` - 已排序的参数键值对（BTreeMap自动排序）
///
/// # 返回值
///
/// 返回Result类型：
/// - Ok(String)：拼接成功
/// - Err(SignError)：拼接失败
fn concat_params(params: &BTreeMap<String, String>) -> Result<String, SignError> {
    // 创建String向量，用于存储每个键值对
    let mut pairs = Vec::new();

    // 遍历所有参数（BTreeMap已按ASCII码排序）
    for (key, value) in params.iter() {
        // 跳过sign参数（与官方文档一致）
        // 官方文档说明：sign参数本身不参与签名计算
        if key == "sign" {
            continue;
        }

        // 注意：根据官方文档，null值不参与拼接，但空字符串需要拼接
        // 由于Rust的String不可能是null，我们只处理空字符串的情况
        // 空字符串需要参与拼接，所以不跳过
        // 这与官方文档完全一致："null值不参与拼接，空字符串需要拼接"

        // 将键值对格式化为"key=value"形式
        pairs.push(format!("{}={}", key, value));
    }

    // 使用"&"连接所有键值对
    let result = pairs.join("&");

    // 返回拼接结果
    Ok(result)
}

/// SHA256哈希计算函数（内部函数）
///
/// 对字符串进行SHA256哈希计算，返回小写十六进制字符串
///
/// # 参数
///
/// * `input` - 需要计算哈希的字符串
///
/// # 返回值
///
/// 返回Result类型：
/// - Ok(String)：哈希计算成功，返回小写十六进制字符串
/// - Err(SignError)：哈希计算失败
fn calculate_sha256_hex(input: &str) -> Result<String, SignError> {
    // 创建SHA256哈希计算器
    let mut hasher = Sha256::new();

    // 更新哈希计算器的输入数据
    // 将字符串转换为字节数组
    hasher.update(input.as_bytes());

    // 完成哈希计算，获取结果
    let hash = hasher.finalize();

    // 将哈希结果转换为小写十六进制字符串
    // hex::encode默认输出小写，与官方文档一致
    let hash_hex = hex::encode(hash);

    // 返回哈希值字符串
    Ok(hash_hex)
}

/// RSA公钥加密函数（内部函数）
///
/// 使用RSA公钥对字符串进行加密
///
/// # 参数
///
/// * `public_key` - RSA公钥
/// * `data` - 需要加密的字符串（SHA256哈希的十六进制字符串）
///
/// # 返回值
///
/// 返回Result类型：
/// - Ok(Vec<u8>)：加密成功，返回加密后的数据
/// - Err(SignError)：加密失败
fn rsa_encrypt_string(public_key: &RsaPublicKey, data: &str) -> Result<Vec<u8>, SignError> {
    // 使用PKCS1v15填充方案进行加密
    // 这是星驿支付使用的RSA加密方案
    // 创建随机数生成器
    let mut rng = rand::thread_rng();

    // 将字符串转换为字节数组进行加密
    // 根据官方文档，加密的是SHA256哈希的十六进制字符串
    let data_bytes = data.as_bytes();

    // 执行RSA公钥加密
    let encrypted = public_key
        .encrypt(&mut rng, Pkcs1v15Encrypt, data_bytes)
        // 如果加密失败，返回错误
        .map_err(|e| SignError::RsaEncryptError(e.to_string()))?;

    // 返回加密结果
    Ok(encrypted)
}

/// Base64编码函数（内部函数）
///
/// 对数据进行Base64编码
///
/// # 参数
///
/// * `data` - 需要编码的数据
///
/// # 返回值
///
/// 返回Result类型：
/// - Ok(String)：编码成功，返回Base64字符串
/// - Err(SignError)：编码失败
fn base64_encode(data: &[u8]) -> Result<String, SignError> {
    // 使用标准Base64编码
    let encoded = STANDARD.encode(data);

    // 返回编码结果
    Ok(encoded)
}

/// 引入rand库用于随机数生成
use rand;

/// 引入hex库用于十六进制编码（用于调试）
use hex;
